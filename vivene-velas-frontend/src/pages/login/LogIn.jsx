...
function Login() {
  ...
  const handleLogin = async () => {
    try {
      const response = await axios.post('http://44.204.200.174/logins/logar', {
        email: email,
        senha: senha
      });
  ...
  const getIdUser = async () => {
    try {
      ...
      const response = await axios.get(`http://44.204.200.174/usuarios/fklogin/${idLogin}?loginId=${idLogin}`);
